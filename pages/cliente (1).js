...
export default function Cliente() {
  ...
  async function avisarStock(producto) {
    ...
    await supabase.from('alertas_stock').insert([{
      producto_id: producto.id,
      producto_nombre: producto.nombre,
      mensaje: `DevRev reportó stock bajo de ${producto.nombre}`,
      leida: false
    }])
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#F7F5F0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ background: 'white', borderRadius: 16, padding: '2.5rem', width: 360, border: '1px solid #E5E2DA', textAlign: 'center' }}>
          ...
          <button onClick={handleLogin} style={{ width: '100%', background: '#2A6B4F', color: 'white', border: 'none', borderRadius: 8, padding: '10px', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", marginTop: 4 }}>
            Ingresar
          </button>
        </div>
      </div>
              ...
              <div>
                {productosConAumento.length > 0 && (
                  <div style={{ background: '#FAEEDA', border: '1px solid #F5C4B3', borderRadius: 10, padding: '.75rem 1rem', display: 'flex', alignItems: 'center', gap: '.75rem', marginBottom: '1.25rem', fontSize: 13 }}>
                    <span>⚠</span>
                    <span><strong>{productosConAumento.length} producto{productosConAumento.length > 1 ? 's' : ''}</strong> con aumento próximo: {productosConAumento.map(p => p.nombre).join(', ')}</span>
                  </div>
                )}
                <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 20, marginBottom: 6 }}>Precios actuales</div>
                <div style={{ fontSize: 13, color: '#7A7568', marginBottom: '1rem' }}>Si notás que un producto está por agotarse, usá el botón para avisarnos.</div>
                {productos.length === 0 ? (
                    ...
                    {productos.map(p => (
                      <div key={p.id} style={{ background: 'white', border: '1px solid #E5E2DA', borderRadius: 12, padding: '1.1rem', position: 'relative' }}>
                        {p.precio_nuevo && p.precio_nuevo > p.precio && (
                          <div style={{ position: 'absolute', top: 10, right: 10, width: 8, height: 8, borderRadius: '50%', background: '#BA7517' }}></div>
                        )}
                        ...
                        <button
                          onClick={() => avisarStock(p)}
                          disabled={avisados[p.id]}
